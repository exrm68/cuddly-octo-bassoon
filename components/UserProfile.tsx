...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  const getLink = () => {
    ...
    // ✅ BUG FIX: Mini App আছে → ?startapp= ; শুধু Bot → ?start= (সঠিক Telegram parameter)
    return app
  ...
  const handleWithdraw = async () => {
    ...
    try {
      const uid = String(tgUser.id);
      // ✅ BUG FIX: Batch write — balance কাটা ও request create atomically হবে
      // যদি যেকোনো একটি fail করে, দুটোই rollback হবে
      const batch = writeBatch(db);

      // 1. Balance কাটো
      batch.update(doc(db,'users', uid), { takaBalance: increment(-amount) });

      // 2. Withdrawal request তৈরি করো
      ...
      // ✅ Commit: দুটো একসাথে হবে অথবা দুটোই হবে না
      await batch.commit();
